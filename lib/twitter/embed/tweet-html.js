...
function getTweetContent($) {
  ...
  tweetContent.children('a').each(function () {
    ...
    if (scribe === 'element:url') {
      ...
      // Quoted Tweet
      if (quotedTweetId) {
        const quotedTweetCard = card.children('[data-scribe="section:quote"]').children('a');

        // console.log('YO', quotedTweetCard.attr('data-tweet-id'));

        if (quotedTweetCard.attr('data-tweet-id') === quotedTweetId) {
          quotedTweet = { id: quotedTweetId, url };
          el.remove();
          return;
        }
      }
